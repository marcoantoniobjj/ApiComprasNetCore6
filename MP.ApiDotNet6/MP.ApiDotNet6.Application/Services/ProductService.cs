...
namespace MP.ApiDotNet6.Application.Services
{
...

    {
        ...
        public async Task<ResultService<ProductDTO>> CreateAsync(ProductDTO productDTO)
        {
            ...
            if (!result.IsValid)
                return ResultService.RequestError<ProductDTO>("Problemas na validacao", result);
        ...
        public async Task<ResultService<ProductDTO>> GetByIdAsync(int id)
        {
            var product = await _productRepository.GetByIdAsync(id);
                if (product == null)
                return ResultService.Fail<ProductDTO> ("Produto nao econtrado");
        ...
        public async Task<ResultService> UpdateAsync(ProductDTO productDTO)
        {
            ...
            if (!validation.IsValid)
                return ResultService.RequestError("Problema de validacao", validation);
